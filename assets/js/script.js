(function () {
  ...
  (function () {
    ...
    var linkRemap = [
      ...
      { prefix: "/contact", target: "/consult/" }
    ];

    var normalizePath = function (hrefValue) {
      if (!hrefValue) {
        return "";
      }
      var trimmed = String(hrefValue).trim();
      if (!trimmed || trimmed.charAt(0) === "#" || trimmed.indexOf("mailto:") === 0 || trimmed.indexOf("tel:") === 0 || trimmed.indexOf("javascript:") === 0) {
        return "";
      }
      try {
    ...
    Array.prototype.slice.call(document.querySelectorAll("a[href]")).forEach(function (linkNode) {
      var currentHref = linkNode.getAttribute("href");
      var path = normalizePath(currentHref);
      if (!path) {
        return;
      }
      for (var i = 0; i < linkRemap.length; i += 1) {
        var rule = linkRemap[i];
        if (path === rule.prefix || path.indexOf(rule.prefix + "/") === 0) {
          linkNode.setAttribute("href", rule.target);
          break;
        }
      }
    });
